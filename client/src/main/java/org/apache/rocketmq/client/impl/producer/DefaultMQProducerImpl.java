...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                this.serviceState = ServiceState.START_FAILED;//初始设置启动失败 启动完成后在设置成功状态
                ...
                //把当前生产者实例 翻入到map中producerTable.putIfAbsent(group, producer)
                ...
                if (startFactory) {
                    mQClientFactory.start();//真正的开始启动
                }

                log.info("the producer [{}] start OK. sendMessageWithVIPChannel={}", this.defaultMQProducer.getProducerGroup(),
                        this.defaultMQProducer.isSendMessageWithVIPChannel());
                this.serviceState = ServiceState.RUNNING;//设置服务启动成功
                break;
            case RUNNING:
            case START_FAILED:
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {//topic和队列不为空
                    ...
                    try {
                        ...
                        switch (communicationMode) {
                            ...
                            case ONEWAY:
                                return null;
                            case SYNC://如果是同步发送
                                if (sendResult.getSendStatus() != SendStatus.SEND_OK) {//发送失败 进行下一次重试
